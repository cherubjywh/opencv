...
void _RGB_ToGray(
	unsigned char *RGB_Arrary,
	unsigned char *Gray_Array,
	int TotalSize)
{
	int i;
		for (i=0;i<TotalSize;i++)
			Gray_Array[i] = (unsigned char) ( 
			                (float)( RGB_Arrary[3*i]   )*0.072169 
			              + (float)( RGB_Arrary[3*i+1] )*0.715160 
						  + (float)( RGB_Arrary[3*i+2] )*0.212671  );
}